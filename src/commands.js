...
const BOT_NAME = "𝔎𝔦𝔫𝔤_𝔅𝔩𝔢𝔰𝔰 𝔗𝔢𝔠𝔥";
const AUTHOR = "ᴋɪɴɢsʟᴇʏ-xᴍᴅ ᴛᴇᴄʜ";
const MENU_IMAGE = "https://i.ibb.co/SXQ0JCYX/jawadmd.jpg";
const OWNER = process.env.OWNER_NUMBER || "233535502036";
...
async function downloadMedia(sock, msg, url, type = "video") {
  ...
  await reply(sock, msg, `⏳ *𝔎𝔦𝔫𝔤_𝔅𝔩𝔢𝔰𝔰 𝔗𝔢𝔠𝔥 is processing your request...*\n🔗 *URL:* ${url}\n🛠️ *Type:* ${type.toUpperCase()}`);
  try {
...
async function handleAI(sock, msg, prompt, mode = "ai", provider = AI_PROVIDER) {
  ...
  try {
    ...
    if (text) await reply(sock, msg, `🤖 *𝔎𝔦𝔫𝔤_𝔅𝔩𝔢𝔰𝔰 𝔗𝔢𝔠𝔥 AI:*\n\n${text}`);
  } catch (err) {
...
async function handleTranscribe(sock, msg, { translateTo } = {}) {
  ...
  try {
    ...
    const tmpFile = path.join(os.tmpdir(), `ᴋɪɴɢsʟᴇʏ-xᴍᴅ ᴛᴇᴄʜ-audio-${Date.now()}.ogg`);
